...
library PowerChangeEpochsLib {
    ...
    /// @notice Finds the index of the epoch in `stakersPowerChangeEpochs` that is first greater than or equal to `epoch`.
    ...
    /// If `epoch` is greater than the last element of `stakersPowerChangeEpochs`, returns `stakersPowerChangeEpochs` length.
    ...
    function findIndex(uint16[] storage stakersPowerChangeEpochs, uint16 epoch, uint256 low, uint256 high) internal view returns (uint256) {
        while (low < high) {
    ...
    /// @notice Finds the index of the epoch in `stakersPowerChangeEpochs` that is first smaller than or equal to `epoch`.
    /// @dev `stakersPowerChangeEpochs` must be sorted in ascending order without duplicates.
    /// If `epoch` is smaller than the first element of `stakersPowerChangeEpochs`, returns 0.
    ...
    /// @param high The upper bound of the search (excluded). Must be in range [1, `stakersPowerChangeEpochs` length].
    /// @return The index of the epoch in `stakersPowerChangeEpochs` that is first smaller than or equal to `epoch`.
    function findNearestIndex(uint16[] storage stakersPowerChangeEpochs, uint16 epoch, uint256 low, uint256 high) internal view returns (uint256) {
